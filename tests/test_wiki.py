...
from browserstack import appium_by
from browserstack.app import screen_language_and_skip_btn
import config
import time


def test_limp_bizkit():
    ...
    with step('Search'):
        ...
        browser.element(appium_by.id('org.wikipedia.alpha:id/search_src_text')).type('Limp Bizkit')
    ...
    with step('Assert close post name'):
        browser.all(appium_by.class_name('android.view.View'))[0].element(appium_by.class_name('android.view.View'))\
            .should(have.text('Limp Bizkit'))
    ...
    with step('Assert contents name'):
        ...
        browser.all(appium_by.id('org.wikipedia.alpha:id/page_toc_item_text'))[0].should(have.text('Limp Bizkit'))
